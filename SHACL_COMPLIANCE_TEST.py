...
# Clean and format
def clean(text):
    return text.replace("_", " ").replace("shape", "").strip()

def get_compliance_test():
    ...
    for policy_file in policy_files:
        ...
        for shacl_file in shacl_files:
            ...
            try:
                ...
                base_policy_clean = clean(base_policy)
                base_shacl_clean = clean(base_shacl)
                # print('base clean', base_policy_clean)
                # # Final summary line
                # summary_line = f"{base_policy_clean} vs {base_shacl_clean}: {result_label.upper()}"
                # print(summary_line)    
                
                summary_line = f"{base_policy_clean} vs {base_shacl_clean}: {result_label}"
    ...
    summary_path = os.path.join(REPORT_FOLDER, SUMMARY_FILE)
